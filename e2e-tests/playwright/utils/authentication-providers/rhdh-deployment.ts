...
class RHDHDeployment {
    ...
    constructor(namespace: string, appConfigMap: string, rbacConfigMap: string, dynamicPluginsConfigMap: string, secretName: string) {
    ...
    async waitForDeploymentReady(timeoutMs: number = 600000): Promise<RHDHDeployment> {
        ...
        const startTime = Date.now();
        const labels = { 'app.kubernetes.io/name': 'backstage', 'app.kubernetes.io/instance': this.instanceName };
        ...
        while (Date.now() - startTime < timeoutMs) {
            try {
                ...
                const isProgressing = conditions.some(
                    condition => condition.type === 'Progressing' && condition.status === 'True' && condition.reason !== 'NewReplicaSetAvailable'
                );
                ...
                if (Date.now() - startTime >= timeoutMs) {
                    throw new Error(`Timeout waiting for deployment to be ready: ${error.message}`);
                }
                await new Promise(resolve => setTimeout(resolve, 5000));
            }
        }
    ...
    async waitForNamespaceActive(timeoutMs: number = 30000): Promise<RHDHDeployment> {
        ...
        while (Date.now() - startTime < timeoutMs) {
            try {
                ...
                if (Date.now() - startTime >= timeoutMs) {
                    throw new Error(`Timeout waiting for namespace to be active: ${error.message}`);
                }
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
        }
    ...
    async createRbacConfig(): Promise<RHDHDeployment> {
        ...
        await this.createConfigMap(this.rbacConfigMap, { 'rbac-policy.csv': this.rbacConfig });
        return this;
    }
    ...
    async updateRbacConfig(): Promise<RHDHDeployment> {
        ...
        await this.updateConfigMap(this.rbacConfigMap, { 'rbac-policy.csv': this.rbacConfig });
        return this;
    }
    ...
    async createDynamicPluginsConfig(): Promise<RHDHDeployment> {
        if (this.isRunningLocal) {
            ...
            this.setAppConfigProperty('dynamicPlugins.rootDirectory', rootDirName + '/dynamic-plugins-root');
            await this.updateAppConfig();
            return this;
        }

        await this.createConfigMap(this.dynamicPluginsConfigMap, { 'dynamic-plugins.yaml': yaml.stringify(this.dynamicPluginsConfig) });
        return this;
    }
    ...
    async updateDynamicPluginsConfig(): Promise<RHDHDeployment> {
        if (this.isRunningLocal) {
            ...
            console.log(`Dynamic plugins config in ${dynamicPluginsConfigPath} has no effect on local deployment. Make sure to update the app-config.test.yaml file to use the dynamic-plugins-root directory and your plugin are already copied there.`);
            return this;
        }

        await this.updateConfigMap(this.dynamicPluginsConfigMap, { 'dynamic-plugins.yaml': yaml.stringify(this.dynamicPluginsConfig) });
        return this;
    }
    ...
    async followPodLogs(searchString: RegExp, podName?: string, podLabels?: any, timeoutMs: number = 300000): Promise<boolean> {
        ...
        try {
            ...
            await log.log(
                ...
                "backstage-backend",
                logStream,
                { follow: true, tailLines: 1, pretty: false, timestamps: false }
            );
            ...
            throw new Error(`Timeout waiting for string "${searchString}" in logs after ${timeoutMs}ms. Error: ${error.body.message}`);
        }
    }
    ...
    setDynamicPluginEnabled(pluginName: string, enabled: boolean): RHDHDeployment {
        const plugin = this.dynamicPluginsConfig.plugins.find((p: any) => p.package == pluginName);
        if (plugin) {
            plugin.disabled = !enabled;
            console.log(`Plugin ${pluginName} has been ${enabled ? 'enabled' : 'disabled'}.`);
        } else {
            this.dynamicPluginsConfig.plugins = [...this.dynamicPluginsConfig.plugins, {
                package: pluginName,
                disabled: !enabled
            }];
            console.log(`Plugin ${pluginName} has been added to the dynamic plugins config and set to ${enabled ? 'enabled' : 'disabled'}.`);
        }
        return this;
    }
    ...
    async enableOIDCLoginWithIngestion(): Promise<RHDHDeployment> {
        ...
        this.setDynamicPluginEnabled("./dynamic-plugins/dist/backstage-community-plugin-catalog-backend-module-keycloak-dynamic", true);
    ...
    async enableMicrosoftLoginWithIngestion(): Promise<RHDHDeployment> {
        ...
        // enable the catalog backend dynamic plugin
        // and set the required configuration properties
        this.setDynamicPluginEnabled("./dynamic-plugins/dist/backstage-plugin-catalog-backend-module-msgraph-dynamic", true);
        this.setAppConfigProperty("catalog.providers", {
            "microsoftGraphOrg": {
                "default": {
                    ...
                    "user": {
                        "filter": "accountEnabled eq true and userType eq 'member' and startswith(displayName,'TEST')"
                    },
                    "group": {
                        "filter": "securityEnabled eq true and mailEnabled eq false and startswith(displayName,'TEST_')\n"
                    },
                    "schedule": {
                        "frequency": "PT1M",
                        "timeout": "PT1M"
    ...
    async checkUserIsIngestedInCatalog(users: string[]) {
        ...
        const catalogUsers: UserEntity[] = response && response.items ? response.items : [];
        ...
        console.log(`Checking ${JSON.stringify(catalogUsersDisplayNames)} contains users ${JSON.stringify(users)}`)
    ...
    async checkGroupIsIngestedInCatalog(groups: string[]) {
        ...
        const catalogGroups: GroupEntity[] = response && response.items ? response.items : [];
        expect(catalogGroups.length).toBeGreaterThan(0);
        const catalogGroupsDisplayNames: string[] = catalogGroups
            .filter((u) => u.spec.profile && u.spec.profile.displayName)
            .map((u) => u.spec.profile.displayName);
        console.log(`Checking ${JSON.stringify(catalogGroupsDisplayNames)} contains groups ${JSON.stringify(groups)}`);
    ...
    async checkUserIsInGroup(user: string, group: string): Promise<boolean> {
        ...
        console.log(`Checking group ${group} (${JSON.stringify(members)}) contains groups ${user}`);
    ...
    async checkGroupIsParentOfGroup(parent: string, child: string): Promise<boolean> {
        ...
        console.log(`Checking children of ${parent} (${JSON.stringify(children)}) contain group ${child}`);
