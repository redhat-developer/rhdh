 ...
 * Enabled only when CI sets PG_UPGRADE_DATA_PROOF=1 (see ocp-pull.sh / ocp-operator.sh).
 */
test.describe("PostgreSQL upgrade data persistence proof", () => {
  test.skip(
    () => process.env.PG_UPGRADE_DATA_PROOF !== "1",
    "Runs only during chart-managed or operator-local PostgreSQL upgrade CI (PG_UPGRADE_DATA_PROOF=1)",
  );
