...
test.describe.serial("Test Adoption Insights", () => {
  test.describe
    ...
    test.describe("Interaction-based tracking tests", () => {
      ...
      // TODO: Fix https://issues.redhat.com/browse/RHDHBUGS-1879 and remove the skip
      test.skip("Changes are Reflecting in panels", async () => {
        ...
        for (const title of titles) {
          ...
          if (
            ...
            const firstRowData = await testHelper.getVisibleFirstRowText(panel);

            if (title === "catalog entities")
              catalogEntitiesFirstEntry = firstRowData;
            else if (title === "techdocs") techdocsFirstEntry = firstRowData;
          }
