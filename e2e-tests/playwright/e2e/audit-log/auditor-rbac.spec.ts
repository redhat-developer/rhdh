...
test.describe('Auditor check for RBAC Plugin', () => {
  ...
  const policyRead = [
    ...
    },
    {
      ...
      meta: { entityRef: `role:${ROLE_NAME}`, queryType: 'by-role', source: 'rest' },
    },
    {
      name: 'by-query',
      call: () =>
  ...
  (['create', 'update', 'delete'] as const).forEach(action => {
    test(`policy-write → ${action}`, async () => {
      ...
      if (action === 'update')
        await rbacApi.updatePolicy(ROLE_NAME, [POLICY_DATA], [{ ...POLICY_DATA, effect: 'deny' }]);
      ...
      await validateRbacLogEvent(
        ...
        { method: httpMethod(action), url },
        { actionType: action, source: 'rest' },
        buildNotAllowedError(action, 'policy', `${ROLE_NAME},policy-entity,read,allow`),
        'failed' as EventStatus,
      );
    });
  });
