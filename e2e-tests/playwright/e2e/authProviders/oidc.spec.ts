...
test.describe('Configure OIDC provider (using RHBK)', async () => {
    ...
    test(`Ingestion of users and groups: verify the user entities and groups are created with the correct relationships`, async () => {
        ...
        expect(await deployment.checkUserIsIngestedInCatalog(["Admin E2e","Atena Minerva","Elio Sole","Tyke Fortuna","Zeus Giove"])).toBe(true);
        expect(await deployment.checkGroupIsIngestedInCatalog(["admins","goddesses","gods"])).toBe(true);
