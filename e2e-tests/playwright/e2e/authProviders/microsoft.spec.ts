...
test.describe('Configure Microsoft Provider', async () => {
    ...
    test.beforeAll(async ({ browser }, testInfo) => {
        ...
        deployment.addSecretData("AUTH_PROVIDERS_AZURE_CLIENT_SECRET", process.env.AUTH_PROVIDERS_AZURE_CLIENT_SECRET);
        deployment.addSecretData("AUTH_PROVIDERS_AZURE_TENANT_ID", process.env.AUTH_PROVIDERS_AZURE_TENANT_ID);
        ...
        const graphClient = new MSGraphClient(process.env.AUTH_PROVIDERS_AZURE_CLIENT_ID, process.env.AUTH_PROVIDERS_AZURE_CLIENT_SECRET, process.env.AUTH_PROVIDERS_AZURE_TENANT_ID);
    ...
    test(`Ingestion of Microsoft users and groups: verify the user entities and groups are created with the correct relationships`, async () => {
        ...
        expect(await deployment.checkUserIsIngestedInCatalog(["TEST Admin","TEST Atena","TEST Elio","TEST Tyke","TEST Zeus"])).toBe(true);
        expect(await deployment.checkGroupIsIngestedInCatalog(["TEST_admins","TEST_goddesses","TEST_gods", "TEST_all"])).toBe(true);
        expect(await deployment.checkUserIsInGroup('admin_rhdhtesting.onmicrosoft.com', 'TEST_admins')).toBe(true);
        expect(await deployment.checkUserIsInGroup('zeus_rhdhtesting.onmicrosoft.com', 'TEST_admins')).toBe(true);
        expect(await deployment.checkUserIsInGroup('atena_rhdhtesting.onmicrosoft.com', 'TEST_goddesses')).toBe(true);
        expect(await deployment.checkUserIsInGroup('tiche_rhdhtesting.onmicrosoft.com', 'TEST_goddesses')).toBe(true);
        expect(await deployment.checkUserIsInGroup('elio_rhdhtesting.onmicrosoft.com', 'TEST_gods')).toBe(true);
        expect(await deployment.checkUserIsInGroup('zeus_rhdhtesting.onmicrosoft.com', 'TEST_gods')).toBe(true);
    ...
    test.afterAll(async () => {
        ...
        const graphClient = new MSGraphClient(process.env.AUTH_PROVIDERS_AZURE_CLIENT_ID, process.env.AUTH_PROVIDERS_AZURE_CLIENT_SECRET, process.env.AUTH_PROVIDERS_AZURE_TENANT_ID);
        await graphClient.removeAppRedirectUrlsAsync([`${backstageUrl}/api/auth/microsoft/handler/frame`]);
    });
});