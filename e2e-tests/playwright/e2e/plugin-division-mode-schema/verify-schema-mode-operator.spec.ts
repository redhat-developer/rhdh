...
test.describe("Verify pluginDivisionMode: schema (Operator)", () => {
  ...
  test.beforeAll(async () => {
    ...
    try {
      ...
      const podName = `backstage-psql-${process.env.RELEASE_NAME || "developer-hub"}-0`;
      const portForwardCmd = `oc port-forward -n ${namespace} ${podName} 5432:5432`;
      
      let troubleshooting = "";
      if (dbHost.includes("svc.cluster.local")) {
        troubleshooting = `Service name detected but connection failed.\n` +
        ...
        troubleshooting = `Connection to localhost failed.\n` +
          `Ensure port-forward is running: ${portForwardCmd}`;
      } else {
        troubleshooting = `Connection failed. Try:\n` +
    ...
    if (oldDbsResult.rows.length > 0) {
      ...
      for (const db of oldDbsResult.rows) {
        try {
          ...
          console.warn(`  Could not drop database ${db.datname}: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }
    ...
    if (dbName !== "postgres") {
      ...
      console.log(`✓ Using default postgres database (schemas will be created here)`);
    }
    ...
    await dbClient.query(`GRANT ALL PRIVILEGES ON DATABASE ${dbName} TO ${dbUser}`);
    ...
    if (dbHost === "localhost" || dbHost === "127.0.0.1") {
      ...
      try {
        ...
        try {
          const services = await kubeClient.coreV1Api.listNamespacedService(namespace);
          const pgServices = services.body.items.filter(s => 
            s.metadata?.name?.includes("psql") || s.metadata?.name?.includes("postgres")
          );
      ...
      sslMode = "disable";
      console.log(`Using service name for pods: ${podDbHost} (original host was ${dbHost} for port-forward)`);
    ...
    try {
      const existingSecret = await kubeClient.coreV1Api.readNamespacedSecret("postgres-cred", namespace);
      const existingHost = existingSecret.body.data?.POSTGRES_HOST 
        ? Buffer.from(existingSecret.body.data.POSTGRES_HOST, "base64").toString()
        : null;
      const existingUser = existingSecret.body.data?.POSTGRES_USER
        ? Buffer.from(existingSecret.body.data.POSTGRES_USER, "base64").toString()
        : null;
      ...
      if (existingHost === podDbHost && existingUser === dbUser && existingDb === dbName) {
        console.log("✓ Postgres credentials secret already configured correctly, skipping update");
    ...
    try {
      ...
      if (!secretAlreadyReferenced) {
        ...
        if (!application.extraEnvs) {
          patch.push({ op: "add", path: "/spec/application/extraEnvs", value: {} });
        }
        if (!extraEnvs.secrets) {
          patch.push({ op: "add", path: "/spec/application/extraEnvs/secrets", value: [] });
        }
        ...
        console.log("✓ Patched Backstage CR to inject postgres-cred secret as environment variables");
        needsRestart = true;
      } else {
      ...
      if (errorMsg.includes("not found") || errorMsg.includes("404")) {
        ...
        console.warn(`[WARNING]  Could not patch Backstage CR to add postgres-cred secret: ${errorMsg}`);
        console.warn(`   RHDH may not read POSTGRES_DB from the secret without this configuration`);
      }
    }
    ...
    if (dbConfigMapExists) {
      await kubeClient.coreV1Api.replaceNamespacedConfigMap(dbConfigMapName, namespace, dbConfigMap);
      console.log(`✓ Updated separate database ConfigMap: ${dbConfigMapName}`);
    } else {
      await kubeClient.coreV1Api.createNamespacedConfigMap(namespace, dbConfigMap);
      console.log(`✓ Created separate database ConfigMap: ${dbConfigMapName}`);
      console.log("   This ConfigMap won't be managed by the operator, so it won't be overwritten.");
    }
    ...
    try {
      ...
      if (!alreadyReferenced) {
        ...
        // Patch with timeout
        const patchPromise = kubeClient.customObjectsApi.patchNamespacedCustomObject(
          backstageCrGroup,
          backstageCrVersion,
          namespace,
          backstageCrPlural,
        ...
        const patchTimeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error("CR patch operation timed out after 30 seconds")), 30000)
        );
      ...
      if (errorMsg.includes("not found") || errorMsg.includes("404")) {
        console.log(`   Backstage CR '${backstageCrName}' not found - this is OK, the separate ConfigMap was created.`);
        console.log(`   If the CR exists with a different name, you may need to manually add the ConfigMap reference.`);
      } else if (errorMsg.includes("timeout")) {
        console.warn(`[WARNING]  CR operation timed out: ${errorMsg}`);
        console.warn(`   Continuing anyway - the separate ConfigMap was created.`);
      } else {
        console.warn(`[WARNING]  Could not patch Backstage CR: ${errorMsg}`);
        console.warn(`   The separate ConfigMap was created, but may not be automatically mounted.`);
        console.warn(`   You may need to manually add it to the Backstage CR's spec.application.appConfig.configMaps.`);
      }
    }
    ...
    const configMapResponse = await kubeClient.getConfigMap(configMapName, namespace);
    ...
    const isAlreadyConfigured = 
      ...
      (currentDbConfig?.connection?.database === `\${POSTGRES_DB}` || currentDbConfig?.connection?.database === dbName);
    
    if (!isAlreadyConfigured) {
      console.log("Also updating main app-config for schema mode (as fallback)...");
      console.log("[WARNING]  NOTE: The operator may overwrite the main ConfigMap, but we've created a separate one.");
      ...
      try {
        ...
        throw new Error(`Generated invalid YAML for app-config: ${error instanceof Error ? error.message : String(error)}`);
      }
      ...
      const verifyConfig = await kubeClient.getConfigMap(configMapName, namespace);
      const verifyConfigKey = Object.keys(verifyConfig.body.data || {}).find((key) =>
        key.includes("app-config"),
      );
      if (!verifyConfigKey) {
        throw new Error(`Could not find app-config key in ConfigMap ${configMapName}. Available keys: ${Object.keys(verifyConfig.body.data || {}).join(", ")}`);
      }
      if (verifyConfig.body.data && verifyConfigKey) {
        try {
          ...
          if (!verifyDbConfig) {
            throw new Error("App-config backend has no 'database' section after update");
          }
          if (verifyDbConfig.pluginDivisionMode !== "schema") {
            throw new Error(`App-config pluginDivisionMode is "${verifyDbConfig.pluginDivisionMode}" instead of "schema"`);
          }
          const expectedDb = `\${POSTGRES_DB}`;
          if (verifyDbConfig.connection?.database !== expectedDb && verifyDbConfig.connection?.database !== dbName) {
            throw new Error(`App-config database is "${verifyDbConfig.connection?.database}" instead of "${expectedDb}" or "${dbName}"`);
          }
          console.log("✓ Verified app-config update is correct");
          console.log(`  pluginDivisionMode: ${verifyDbConfig.pluginDivisionMode}`);
          console.log(`  ensureExists: ${verifyDbConfig.ensureExists ?? 'not set (defaults to true)'}`);
          console.log(`  ensureSchemaExists: ${verifyDbConfig.ensureSchemaExists ?? 'not set (defaults to false)'}`);
          ...
          throw new Error(`Failed to verify app-config update: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
        }
      }
    } else {
      console.log("✓ RHDH is already configured for schema mode in app-config");
      console.log("[WARNING]  However, RHDH needs to be restarted to actually use schema mode.");
      console.log("   If RHDH was already running, it may still be using separate databases.");
    ...
    try {
      const deployment = await kubeClient.appsApi.readNamespacedDeployment(deploymentName, namespace);
      ...
      if (deploymentReady) {
        console.log(`✓ Deployment is already running (${readyReplicas} ready, ${availableReplicas} available)`);
      } else {
    ...
    if (needsRestart) {
      ...
      console.log("Waiting additional time for RHDH to create schemas (this may take 1-2 minutes)...");
      ...
      console.log("[WARNING]  No config changes needed, but deployment is not ready.");
      console.log("   Waiting for deployment to become ready...");
      await kubeClient.waitForDeploymentReady(deploymentName, namespace, 1, 120000);
    } else {
      console.log("✓ No restart needed - configuration already applied and deployment is ready");
    }
  });
  ...
  test("Verify schemas were created", async () => {
    ...
    const kubeClient = new KubeClient();
    await kubeClient.waitForDeploymentReady(deploymentName, namespace, 1, 180000);

    // Wait a bit more for RHDH to fully initialize plugins
    console.log("Waiting for RHDH to fully initialize plugins (schemas are created lazily)...");
    ...
    console.log(`Connecting to database ${dbName} as admin user to check for schemas...`);
    const adminClient = new Client({
      host: dbHost,
      port: 5432,
      user: dbAdminUser,
    ...
    try {
      ...
      console.log(`✓ Verified test user is connected to database: ${dbCheck.rows[0].current_database}`);
    } catch (connectError) {
      const errorMsg = connectError instanceof Error ? connectError.message : String(connectError);
      console.warn(`[WARNING]  Warning: Test user ${dbUser} cannot connect: ${errorMsg}`);
      console.warn(`   Will use admin user to check schemas instead.`);
      console.warn(`   This might mean RHDH is using a different user than the test user.`);
    }
    ...
    try {
      ...
      console.log(`✓ Verified we're connected to database: ${dbCheck.rows[0].current_database}`);
    } catch (connectError) {
      const errorMsg = connectError instanceof Error ? connectError.message : String(connectError);
      throw new Error(
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      try {
        ...
        const expected = ["catalog", "scaffolder", "auth", "permission", "search"];
        found = expected.filter((s) => schemas.includes(s));
        
        // Also get schema owners for better diagnostics
        const schemaOwners = await adminClient.query<{ schema_name: string; schema_owner: string }>(`
        ...
        if (found.length >= 3) {
          console.log(`✓ Found ${found.length} plugin schemas: ${found.join(", ")}`);
        ...
        console.log(`Waiting for schemas to be created... (${elapsed}s elapsed, found: ${found.join(", ") || "none"})`);
        if (schemas.length > 0) {
        ...
        if (currentDb.rows[0].current_database !== dbName) {
          console.warn(`[WARNING]  Warning: Connected to database '${currentDb.rows[0].current_database}' instead of '${dbName}'`);
        }
      } catch (queryError) {
        ...
        try {
          ...
          if (podsResponse.body.items.length > 0) {
            ...
            if (podName) {
              ...
              try {
                const pod = await kubeClient.coreV1Api.readNamespacedPod(podName, namespace);
                const backstageContainer = pod.body.spec?.containers?.find(c => c.name === "backstage-backend");
                if (backstageContainer) {
                  const postgresDbEnv = backstageContainer.env?.find(e => e.name === "POSTGRES_DB");
                  if (postgresDbEnv) {
                    const dbValue = postgresDbEnv.value || (postgresDbEnv.valueFrom?.secretKeyRef ? "<from secret>" : "NOT SET");
                    console.log(`  Pod POSTGRES_DB env var: ${dbValue}`);
                    if (postgresDbEnv.valueFrom?.secretKeyRef) {
                      console.log(`    Secret: ${postgresDbEnv.valueFrom.secretKeyRef.name}, Key: ${postgresDbEnv.valueFrom.secretKeyRef.key}`);
                    }
                  } else {
              ...
              if (logs.body) {
                ...
                if (relevantLogs.length > 0) {
                  console.log(`  Relevant pod logs (last ${Math.min(10, relevantLogs.length)} lines):`);
    ...
    if (found.length < 3) {
      ...
      const allSchemaOwners = await adminClient.query<{ schema_name: string; schema_owner: string }>(`
        SELECT schema_name, schema_owner 
        FROM information_schema.schemata 
        WHERE schema_name NOT IN ('pg_catalog', 'information_schema', 'pg_toast', 'pg_temp_1', 'pg_toast_temp_1')
          AND schema_name NOT LIKE 'pg_%'
      ...
      try {
        ...
        if (!verifyConfigKey) {
          ...
          try {
            ...
            if (verifyConfig.body.data[verifyConfigKey]) {
              const configPreview = verifyConfig.body.data[verifyConfigKey].substring(0, Math.min(500, verifyConfig.body.data[verifyConfigKey].length));
              configCheck += `\n  Config preview (first 500 chars):\n${configPreview}...`;
            }
          }
        }
        ...
        // Check secret
        try {
          const secret = await kubeClient.coreV1Api.readNamespacedSecret("postgres-cred", namespace);
          const secretData = secret.body.data || {};
          const secretHost = secretData.POSTGRES_HOST ? Buffer.from(secretData.POSTGRES_HOST, "base64").toString() : "NOT SET";
          const secretUser = secretData.POSTGRES_USER ? Buffer.from(secretData.POSTGRES_USER, "base64").toString() : "NOT SET";
          const secretPort = secretData.POSTGRES_PORT ? Buffer.from(secretData.POSTGRES_PORT, "base64").toString() : "NOT SET";
          const secretDb = secretData.POSTGRES_DB ? Buffer.from(secretData.POSTGRES_DB, "base64").toString() : "NOT SET";
          const secretPassword = secretData.POSTGRES_PASSWORD ? "***SET***" : "NOT SET";
      ...
      const schemaOwnerInfo = allSchemaOwners.rows.length > 0 
  ...
  test("Verify no separate plugin databases", async () => {
    ...
    console.log("Waiting for RHDH to fully restart and close old database connections...");
    await new Promise((resolve) => setTimeout(resolve, 30000));

    console.log("Checking for leftover plugin databases from before schema mode...");
    ...
    if (oldDbsResult.rows.length > 0) {
      console.log(`Found ${oldDbsResult.rows.length} leftover plugin databases, attempting to clean up...`);
      for (const db of oldDbsResult.rows) {
        try {
          await client.query(`
            SELECT pg_terminate_backend(pid)
          ...
          if (connCount > 0) {
            console.warn(`  Database ${db.datname} has ${connCount} active connection(s) - may still be in use by RHDH`);
          }
        }
      }
    }
    ...
    if (pluginDbs.length > 0) {
        ...
        pluginDbs.map(async (db) => {
          try {
            const connCheck = await client.query<{ count: string }>(`
              ...
              WHERE datname = $1
            `, [db]);
            return { db, activeConnections: parseInt(connCheck.rows[0].count, 10) };
          } catch {
      ...
      if (activeDbs.length > 0) {
        ...
        console.warn(`[WARNING]  Found ${pluginDbs.length} leftover plugin databases with no active connections: ${pluginDbs.join(", ")}`);
        console.warn(`   These appear to be from before schema mode was enabled. They should be cleaned up.`);
      }
    }
    
    console.log("✓ No separate plugin databases found (schema mode is working)");
    await client.end();
  });
  ...
  test("Verify RHDH is accessible", async ({ page }) => {
    ...
    if (!baseUrl) {
      console.warn("BASE_URL environment variable is not set. Attempting to determine from OpenShift Route...");
      const kubeClient = new KubeClient();
      try {
        const releaseName = process.env.RELEASE_NAME || "developer-hub";
        const routeNames = [
        ...
        if (routeHost) {
          ...
          console.warn("Could not find OpenShift Route. Please set BASE_URL environment variable.");
          throw new Error(
