...
test.describe("Verify pluginDivisionMode: schema (Helm Chart)", () => {
  ...
  test.beforeAll(async () => {
    ...
    console.log(`✓ Created/verified bn_backstage database (Helm chart default, but we'll use ${dbName} via POSTGRES_DB)`);
    ...
    await adminClient.query(`GRANT CONNECT ON DATABASE bn_backstage TO ${dbUser}`).catch(() => {});
    ...
    try {
      ...
      await bnDbClient.query(`GRANT CREATE ON DATABASE bn_backstage TO ${dbUser}`);
      ...
      await bnDbClient.query(`GRANT ALL PRIVILEGES ON DATABASE bn_backstage TO ${dbUser}`);
      ...
      console.warn(`[WARNING]  Could not grant permissions on bn_backstage database: ${err instanceof Error ? err.message : String(err)}`);
    }
      ...
      try {
        ...
        try {
          ...
          const pgServices = services.body.items.filter(s => 
            s.metadata?.name?.includes("postgresql") || s.metadata?.name?.includes("postgres")
          );
    ...
    try {
      const existingSecret = await kubeClient.coreV1Api.readNamespacedSecret(secretName, namespace);
      ...
      const existingPassword = existingSecret.body.data?.password 
        ...
        ? Buffer.from(existingSecret.body.data["postgres-password"], "base64").toString()
        : existingSecret.body.data?.["postgresql-password"]
        ? Buffer.from(existingSecret.body.data["postgresql-password"], "base64").toString()
        : null;
      
      if (existingPassword === dbPassword) {
        console.log(`✓ Helm chart secret ${secretName} already has correct password`);
    ...
    if (secretNeedsUpdate) {
      try {
        const helmSecret = await kubeClient.coreV1Api.readNamespacedSecret(secretName, namespace);
        const updatedSecret = helmSecret.body;
        if (!updatedSecret.data) updatedSecret.data = {};
        
        // Update password in all possible keys
        updatedSecret.data.password = Buffer.from(dbPassword).toString("base64");
        updatedSecret.data["postgres-password"] = Buffer.from(dbPassword).toString("base64");
        updatedSecret.data["postgresql-password"] = Buffer.from(dbPassword).toString("base64");
        ...
        updatedSecret.data["postgres-db"] = Buffer.from(dbName).toString("base64");
        updatedSecret.data["postgresql-db"] = Buffer.from(dbName).toString("base64");
        
        delete updatedSecret.metadata?.resourceVersion;
        await kubeClient.coreV1Api.replaceNamespacedSecret(secretName, namespace, updatedSecret);
        console.log(`✓ Updated Helm chart secret ${secretName} with test user password`);
        needsRestart = true; // Helm chart deployments need restart to pick up secret changes
      } catch (error) {
        console.warn(`[WARNING]  Could not update Helm chart secret ${secretName}: ${error instanceof Error ? error.message : String(error)}`);
        console.warn(`   You may need to manually update the secret or ensure the Helm chart user password matches the test user password.`);
      }
    }
    ...
    try {
      ...
      const backstageContainerIndex = containers.findIndex(c => c.name === "backstage-backend");
      ...
      if (backstageContainer) {
        ...
        if (!hasPostgresDb) {
          ...
          try {
            ...
            console.log("✓ Added POSTGRES_DB environment variable to deployment via API");
            needsRestart = true;
          } catch (apiError) {
            ...
            console.log("Attempting to patch deployment using oc command as fallback...");
            ...
            try {
              ...
              console.log("✓ Added POSTGRES_DB environment variable to deployment via oc command");
              
              // Verify the patch was applied
              await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait 2 seconds for API to sync
              const verifyDeployment = await kubeClient.appsApi.readNamespacedDeployment(deploymentName, namespace);
              const verifyContainer = verifyDeployment.body.spec?.template?.spec?.containers?.find(c => c.name === "backstage-backend");
              const verifyEnv = verifyContainer?.env || [];
              const verifyPostgresDb = verifyEnv.find(e => e.name === "POSTGRES_DB");
              
              if (!verifyPostgresDb) {
                throw new Error("Patch command succeeded but POSTGRES_DB was not found in deployment after patching");
              }
              console.log("✓ Verified POSTGRES_DB environment variable was added to deployment");
              needsRestart = true;
            } catch (ocError) {
              const ocErrorMsg = ocError instanceof Error ? ocError.message : String(ocError);
              console.error(`[ERROR]  Failed to patch deployment via oc command: ${ocErrorMsg}`);
              console.error(`   Please manually run this command to add POSTGRES_DB:`);
              console.error(`   oc patch deployment ${deploymentName} -n ${namespace} --type='json' -p='[{"op":"add","path":"/spec/template/spec/containers/0/env/-","value":{"name":"POSTGRES_DB","valueFrom":{"secretKeyRef":{"name":"${secretName}","key":"POSTGRES_DB"}}}}]'`);
              throw new Error(`Failed to add POSTGRES_DB environment variable to deployment. Both API and oc command failed.`);
            }
          }
        } else {
          console.log("✓ POSTGRES_DB environment variable already exists in deployment");
          ...
          if (postgresDbEnv?.valueFrom?.secretKeyRef) {
            ...
            if (secretRef.name !== secretName || secretRef.key !== "POSTGRES_DB") {
              console.warn(`[WARNING]  POSTGRES_DB env var references secret ${secretRef.name}/${secretRef.key}, expected ${secretName}/POSTGRES_DB`);
            }
          }
        }
      }
      ...
      if (!errorMsg.includes("Failed to add POSTGRES_DB")) {
        console.warn(`[WARNING]  Could not check/add POSTGRES_DB to deployment: ${errorMsg}`);
        console.warn(`   The deployment might need POSTGRES_DB as an environment variable for Backstage to use the correct database.`);
      } else {
    ...
    try {
      ...
      if (!verifyPostgresDb) {
        console.error(`[ERROR]  Deployment does not have POSTGRES_DB environment variable!`);
        console.error(`   Backstage will not be able to resolve \${POSTGRES_DB} in app-config.`);
      }
    } catch (validationError) {
      // Only log if it's a critical error
      const errorMsg = validationError instanceof Error ? validationError.message : String(validationError);
      if (!errorMsg.includes("not found")) {
        console.warn(`[WARNING]  Could not validate configuration: ${errorMsg}`);
      }
    }
    ...
    if (needsRestart) {
      ...
      console.log("   Note: Helm chart deployments require a restart to pick up ConfigMap and secret changes.");
      try {
        ...
        if (errorMsg.includes("ephemeral volume") || errorMsg.includes("persistentvolumeclaim")) {
          console.warn("[WARNING]  Restart failed due to cluster storage issue (PVC cannot be created).");
          console.warn("   This is a transient cluster issue, not a test failure.");
          console.warn("   For Helm chart deployments, ConfigMap changes may be picked up without restart.");
          console.warn("   Continuing with test - if schemas aren't created, you may need to manually restart the deployment.");
          // Continue with test - wait a bit for any config changes to take effect
          console.log("Waiting for RHDH to apply configuration changes (if any)...");
      ...
      console.warn("[WARNING]  No config changes needed, but deployment is not ready. Waiting...");
      await kubeClient.waitForDeploymentReady(deploymentName, namespace, 1, 120000, 10000, podLabelSelector);
    } else {
  ...
  test("Verify schemas were created", async () => {
    ...
    try {
      ...
      if (!verifyPostgresDb) {
        ...
        console.error(`   oc patch deployment ${deploymentName} -n ${namespace} --type='json' -p='[{"op":"add","path":"/spec/template/spec/containers/0/env/-","value":{"name":"POSTGRES_DB","valueFrom":{"secretKeyRef":{"name":"${secretName}","key":"POSTGRES_DB"}}}}]'`);
        throw new Error(`Deployment ${deploymentName} must have POSTGRES_DB environment variable set`);
      }
    } catch (validationError) {
      const errorMsg = validationError instanceof Error ? validationError.message : String(validationError);
      if (errorMsg.includes("must have") || errorMsg.includes("does not have")) {
        throw validationError; // Re-throw critical validation errors
      }
    }
    
    await kubeClient.waitForDeploymentReady(deploymentName, namespace, 1, 180000, 10000, podLabelSelector);
    ...
    if (found.length < 3) {
      ...
      try {
        ...
        // Check Helm secret
        try {
          const secret = await kubeClient.coreV1Api.readNamespacedSecret(secretName, namespace);
