...
test.describe("Admin > Extensions > Catalog", () => {
  ...
  const commonHeadings = ["Versions", "Author", "Tags", "Category", "Publisher", "Support Provider"];
  const supportTypeOptions = ["Generally available", "Certified", "Tech preview", "Dev preview", "Community plugin"];
  ...
  test("Verify category filters in extensions", async ({ page }, testInfo) => {
  ...
  test("Verify Generally available badge in extensions", async ({ page }) => {
    ...
    await expect(page.getByLabel("Generally available (GA) and supported by Red Hat").first()).toBeVisible();
    await expect(extensions.badge.first()).toBeVisible();
    await extensions.badge.first().hover();
    await uiHelper.verifyTextInTooltip("Generally available (GA) and supported by Red Hat");
    
    await uiHelper.clickLink("Read more");
    await expect(
      page.getByLabel("Production-ready and supported by Red Hat").getByText("Generally available (GA)")
    ).toBeVisible();
  ...
  test("Verify community plugin badge in extensions", async ({ page }) => {
    ...
    await uiHelper.clickLink("Read more");
    await expect(
      page.getByLabel('Open-source plugins, no official support').getByText('Community plugin')
    ).toBeVisible();
