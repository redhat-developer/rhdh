...
export class CatalogImport {
  ...
  /**
   * Analyzes the given URL and returns whether the component is already registered.
   *
   * @param url - The component URL to analyze
   * @returns boolean indicating if the component is already registered
   */
  async checkIfComponentIsAlreadyRegistered(url: string): Promise<boolean> {
    await this.analyzeAndWait(url);
    return this.isComponentAlreadyRegistered();
  }
  ...
  /**
   * Fills the component URL and clicks the "Analyze" button (no validation waits).
   *
   * @param url - The component URL to analyze
   */
