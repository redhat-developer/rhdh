...
export class Orchestrator {
  ...
  async validateWorkflowStatusDetails(status = "Completed") {
    ...
    if (status === "Running") {
      ...
      const runningBtn = this.page
        .locator(".MuiGrid-root > div:nth-child(2)")
        .first();
      await expect(runningBtn).toHaveText(/Running/i);
      await expect(runningBtn.getByRole("progressbar")).toBeVisible();
  ...
  async validateCurrentWorkflowStatus(status = "Completed", timeout = 90000) {
    await expect(this.page.getByText(`${status}`, { exact: true })).toBeVisible(
      {
        timeout,
      },
