 ...
 * carries the pluginConfig blocks merged by install-dynamic-plugins. The local
 * E2E overlay must come after that generated config: install-dynamic-plugins
 * may contribute its own `app.extensions` array, and config arrays replace
 * rather than merge. Keeping the test overlay last preserves the NFS route,
 * language API, and other deterministic test-only settings. `extraConfigs`
