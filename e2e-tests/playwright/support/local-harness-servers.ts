 ...
 * come after the static app-config.dynamic-plugins.yaml to win. The local E2E
 * overlay must come after the generated config: install-dynamic-plugins may
 * contribute its own `app.extensions` array, and config arrays replace rather
 * than merge. Keeping the test overlay last preserves the NFS route, language
 * API, and other deterministic test-only settings. `extraConfigs` lands last
