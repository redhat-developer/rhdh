...
if [[ -n "${TAG_NAME}" && -z "${QUAY_REPO}" ]]; then
  log::error "--tag requires --quay-repo to be set."
  exit 1
fi

# --- Build env overrides using jq for safe JSON escaping ---
add_env() { jq --arg k "$1" --arg v "$2" '. + {($k): $v}'; }

envs="{}"
[[ -n "${QUAY_REPO}" ]] && envs=$(echo "${envs}" | add_env "MULTISTAGE_PARAM_OVERRIDE_QUAY_REPO" "${QUAY_REPO}")
[[ -n "${TAG_NAME}" ]] && envs=$(echo "${envs}" | add_env "MULTISTAGE_PARAM_OVERRIDE_TAG_NAME" "${TAG_NAME}")
[[ -n "${GITHUB_ORG_NAME}" ]] && envs=$(echo "${envs}" | add_env "MULTISTAGE_PARAM_OVERRIDE_GITHUB_ORG_NAME" "${GITHUB_ORG_NAME}")
[[ -n "${GITHUB_REPOSITORY_NAME}" ]] && envs=$(echo "${envs}" | add_env "MULTISTAGE_PARAM_OVERRIDE_GITHUB_REPOSITORY_NAME" "${GITHUB_REPOSITORY_NAME}")
[[ -n "${RELEASE_BRANCH_NAME}" ]] && envs=$(echo "${envs}" | add_env "MULTISTAGE_PARAM_OVERRIDE_RELEASE_BRANCH_NAME" "${RELEASE_BRANCH_NAME}")
envs=$(echo "${envs}" | add_env "MULTISTAGE_PARAM_OVERRIDE_SKIP_SEND_ALERT" "${SKIP_SEND_ALERT}")

# --- Build JSON payload ---
payload=$(jq -n --arg job "${JOB_NAME}" --argjson envs "${envs}" \
  '{job_name: $job, job_execution_type: "1", pod_spec_options: {envs: $envs}}')
...
# Try to extract the job ID and fetch status.
...
if [[ -n "${job_id}" ]]; then
  ...
  for _ in $(seq 1 5); do
    ...
    if [[ -n "${status_response}" ]] && echo "${status_response}" | jq . &> /dev/null; then
      ...
      job_status=$(echo "${status_response}" | jq -r '.job_status // "unknown"')
      [[ -n "${job_url}" ]] && break
    fi
    sleep 2
  done
