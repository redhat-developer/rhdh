...
# Add !<plugin-path> suffixes to ghcr.io OCI URLs to skip skopeo inspect,
# which times out on OSD-GCP due to unreliable ghcr.io connectivity.
# {{inherit}} plugins are excluded because the init container resolves their
# path automatically from dynamic-plugins.default.yaml.
add_ghcr_plugin_path_suffix() {
  local file=$1
  log::info "Adding !<plugin-path> suffixes to ghcr.io OCI URLs in ${file}"
  yq -i '
    (.global.dynamic.plugins[].package
      | select(test("ghcr\\.io"))
      | select(test("!") | not)
      | select(test("inherit") | not)) |=
      (. as $pkg |
       ($pkg | split("/") | .[-1] | split(":") | .[0] | split("@") | .[0]) as $name |
       $pkg + "!" + $name)
  ' "${file}"
}
...
initiate_operator_deployments_osd_gcp() {
  ...
  # Merge base values with OSD-GCP diff file, add !plugin-path to ghcr.io URLs, then create ConfigMap
  ...
  add_ghcr_plugin_path_suffix "/tmp/merged-values_showcase_OSD-GCP.yaml"
  ...
  # Merge RBAC values with OSD-GCP diff file, add !plugin-path to ghcr.io URLs, then create ConfigMap
  ...
  add_ghcr_plugin_path_suffix "/tmp/merged-values_showcase-rbac_OSD-GCP.yaml"
