...
# Download schemas to a private temporary directory (ajv doesn't support remote schemas well)
SCHEMA_DIR=$(mktemp -d "${TMPDIR:-.}/rhdh-schemas.XXXXXX")
...
curl -s "https://raw.githubusercontent.com/redhat-developer/rhdh-plugins/main/workspaces/extensions/json-schema/packages.json" \
  -o "$SCHEMA_DIR/packages.json"
curl -s "https://raw.githubusercontent.com/redhat-developer/rhdh-plugins/main/workspaces/extensions/json-schema/plugins.json" \
  -o "$SCHEMA_DIR/plugins.json"

# Convert YAML to JSON and validate Package against local schema
echo "Validating packages/{plugin-name}.yaml..."
yq eval packages/{plugin-name}.yaml -o json > "$SCHEMA_DIR/package-temp.json"
ajv validate -s "$SCHEMA_DIR/packages.json" -d "$SCHEMA_DIR/package-temp.json"

# Convert YAML to JSON and validate Plugin against local schema
echo "Validating plugins/{plugin-name}.yaml..."
yq eval plugins/{plugin-name}.yaml -o json > "$SCHEMA_DIR/plugin-temp.json"
ajv validate -s "$SCHEMA_DIR/plugins.json" -d "$SCHEMA_DIR/plugin-temp.json"
```
