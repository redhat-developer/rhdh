        ...
        return async (
          ...
          if (!profile.email) {
            ...
            if (res.data.length == 0) {
              throw new Error('Failed to sign-in, unable to resolve user identity. Could not get emails in user profile.')
            }

            const usersWithVerifiedEmails = res.data.filter(email => email.verified);
            if (!usersWithVerifiedEmails) {
              throw new Error('Failed to sign-in, unable to resolve user identity. No verified emails were found.')
            }
          ...
          throw new Error('Failed to sign-in, unable to resolve user identity. Please verify that your catalog contains the expected user entities that would match your configured sign-in resolver.');
        };
      },
    });
}
