...
#             tar.extractall(os.path.abspath(tmpfiles), members=filesToExtract, filter='tar')

    def extract_plugin(self, tar_file: str, plugin_path: str) -> None:
        """
        Extract only the subtree rooted at `plugin_path` into a temporary dir
        created inside `self.destination`, then atomically replace the plugin dir.
        """
        tmp_root = tempfile.mkdtemp(dir=self.destination, prefix='_tmpfiles_')
        try:
            with tarfile.open(tar_file, 'r:gz') as tar:
                files_to_extract = []
                prefix = plugin_path if plugin_path.endswith('/') else plugin_path + '/'
                for member in tar.getmembers():
                    ...
                    if member.islnk() or member.issym():
                        linkname = getattr(member, 'linkpath', member.linkname)
                        realpath = os.path.realpath(os.path.join(plugin_path, *os.path.split(linkname)))
                        if not realpath.startswith(plugin_path):
                            print(f'\t==> WARNING: skipping link outside of the archive: {member.name} -> {linkname}', flush=True)
                            continue

                    m = copy.copy(member)
                    m.name = m.name[len(prefix):]  # strip leading plugin_path/
                    files_to_extract.append(m)

                for member in files_to_extract:
                    tar.extract(member, path=tmp_root, filter='tar')

            final_dir = os.path.join(self.destination, plugin_path)
            os.makedirs(os.path.dirname(final_dir), exist_ok=True)

            if os.path.exists(final_dir):
                shutil.rmtree(final_dir, ignore_errors=True)

            os.replace(tmp_root, final_dir)
        ...
        with tarfile.open(archive, 'r:gz') as tar:
            for member in tar.getmembers():
                if member.isreg():
                    ...
                    tar.extract(member, path=tmpfiles, filter='tar')
