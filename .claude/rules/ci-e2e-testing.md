...
The package aliases below document project mappings and require an environment already prepared by
CI. For local runs, invoke the equivalent project through `local-test.sh` so Bitwarden secrets and
certificate files are prepared correctly, for example:
...
# Utility scripts
yarn lint      # Lint checking
yarn lint:fix  # Lint fixing
yarn fmt:check # Formatting check
yarn fmt       # Formatting fix
```
...
Store local test secrets in the approved Bitwarden collection and CI secrets in the configured
OpenShift CI secret store.
...
- **Trace analysis**: `npx playwright trace test-results/TEST_PATH/trace.zip`
