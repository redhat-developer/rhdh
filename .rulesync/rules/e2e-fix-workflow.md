...
If you're already on a feature branch and need to determine the base release branch:

```bash
# Find which upstream branch the current branch is based on
git branch -r --contains "$(git merge-base HEAD upstream/main)" | grep 'upstream/' | head -1 | sed 's|.*upstream/||'
```
