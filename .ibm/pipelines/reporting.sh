...
save_status_deployment_namespace() {
  ...
  log::debug_stdout "Saving STATUS_DEPLOYMENT_NAMESPACE[\"${current_deployment}\"]=${current_namespace}"
...
save_status_failed_to_deploy() {
  local current_deployment=$1
  local status=$2
  log::debug_stdout "Saving STATUS_FAILED_TO_DEPLOY[\"${current_deployment}\"]=${status}"
...
save_status_test_failed() {
  local current_deployment=$1
  local status=$2
  log::debug_stdout "Saving STATUS_TEST_FAILED[\"${current_deployment}\"]=${status}"
...
save_status_number_of_test_failed() {
  local current_deployment=$1
  local number=$2
  log::debug_stdout "Saving STATUS_NUMBER_OF_TEST_FAILED[\"${current_deployment}\"]=${number}"
...
save_overall_result() {
  ...
  log::info_stdout "Saving OVERALL_RESULT=${OVERALL_RESULT}"
...
save_is_openshift() {
  ...
  log::debug_stdout "Saving IS_OPENSHIFT=${is_openshift}"
...
save_container_platform() {
  ...
  log::debug_stdout "Saving CONTAINER_PLATFORM=${container_platform}"
  log::debug_stdout "Saving CONTAINER_PLATFORM_VERSION=${container_platform_version}"
...
get_artifacts_url() {
  ...
  if [ -z "${namespace}" ]; then
    log::warn_stdout "namespace parameter is empty (this is expected only for top level artifacts directory)"
  fi
...
save_data_router_junit_results() {
  ...
  log::success_stdout "JUnit results for ${namespace} adapted to Data Router format and saved to ARTIFACT_DIR and copied to SHARED_DIR"
  log::debug_stdout "Shared directory contents:"
  ls -la "${SHARED_DIR}"
}
