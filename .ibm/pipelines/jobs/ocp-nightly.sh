...
run_standard_deployment_tests() {
  ...
  check_and_test "${RELEASE_NAME}" "${NAME_SPACE}" "showcase" "${url}"
  local rbac_url="https://${RELEASE_NAME_RBAC}-developer-hub-${NAME_SPACE_RBAC}.${K8S_CLUSTER_ROUTER_BASE}"
  check_and_test "${RELEASE_NAME_RBAC}" "${NAME_SPACE_RBAC}" "showcase-rbac" "${rbac_url}"
}
...
run_sanity_plugins_check() {
  ...
  check_and_test "${RELEASE_NAME}" "${NAME_SPACE_SANITY_PLUGINS_CHECK}" "showcase-sanity-plugins" "${sanity_plugins_url}"
}
